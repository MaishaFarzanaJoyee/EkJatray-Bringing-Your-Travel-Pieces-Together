...
export async function searchAccommodations(location) {
  const response = await api.get("/api/booking/accommodations", {
    params: { location },
  });
  return response.data;
}
...
export async function searchWellnessCenters(location) {
  const response = await api.get("/api/booking/wellness", {
    params: { location },
  });
  return response.data;
}
...
export async function bookLodging(hotelId) {
  const response = await api.post("/api/booking/plan/book-lodging", { hotelId });
  return response.data;
}

export async function bookWellness(wellnessId) {
  const response = await api.post("/api/booking/plan/book-wellness", { wellnessId });
  return response.data;
}

export async function getUnifiedItinerary() {
  const response = await api.get("/api/booking/plan/unified-itinerary");
  return response.data;
}

export async function deleteLodging(id) {
  const response = await api.delete(`/api/booking/plan/lodging/${id}`);
  return response.data;
}
